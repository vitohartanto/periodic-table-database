...
PRINT_ELEMENT(){
  ...
  if [[ ! $CHECK_INPUT =~ ^[0-9]+$ ]]
  then
    ATOMIC_NUMBER=$(echo $($PSQL "SELECT atomic_number FROM elements WHERE name='$CHECK_INPUT' OR symbol='$CHECK_INPUT';") | sed 's/ //g')
  else
    ATOMIC_NUMBER=$(echo $($PSQL "SELECT atomic_number FROM elements WHERE atomic_number=$CHECK_INPUT;") | sed 's/ //g')
  fi
